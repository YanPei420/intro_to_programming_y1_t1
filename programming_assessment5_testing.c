...
void check_collision(Game* game, Player *player, Robot *robot, Position *mines, int *mine_count, CrossObstacle* obstacle, Position *person);
...
// Dummy function 
void draw_title_screen(Player *player) {
    (void) player; // To avoid unused parameter warning
    if (player == NULL) {
        return;
    }
    int start_game;
    ...
    //get input
    start_game = getch();
    // Wait for user to press 'm' to start
    nodelay(stdscr, FALSE);        // make getch() blocking
    while (start_game != 'm') {
        nodelay(stdscr, TRUE);     // restore non-blocking //block until m is pressed
        clear();
        mvprintw(9, 1, "Enter your name: ");
        refresh();
        echo();
        char name[MAX_NAME] = {0};      // initialising name buffer
        int ret = getnstr(name, MAX_NAME - 1);    //-1 to leave space for 0/
        noecho();
        if (ret != ERR) {
            strncpy(player->name, name, MAX_NAME-1); // successful input
            player->name[MAX_NAME-1] = '\0'; // Ensure null-termination
        }else{
            strncpy(player->name, "Player1", MAX_NAME-1); // default name if input fails
            player->name[MAX_NAME-1] = '\0'; // Ensure null-termination
        }
        clear();
        refresh();
    ...
    if (ret != ERR) {       // successful input (ERR when user presses ESC or on timeout)
        strncpy(player->name, name, MAX_NAME-1); // successful input
        player->name[MAX_NAME-1] = '\0'; // Ensure null-termination
    }else{
        strncpy(player->name, "Player1", MAX_NAME-1); // default name if input fails
...
void check_collision(Game* game, Player *player, Robot *robot, Position *mines, int *mine_count, CrossObstacle* obstacle, Position *person) {
    ...
    //display current lives, score and level on the screen
    void update_UI(Player *player, Robot *robot, Position *person, Position *mines, int mine_count, Game *game);
    if (player == NULL || robot == NULL) {        //check for null pointers
        return;
    }
    // Update player stats
